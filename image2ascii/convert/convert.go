...
import (
	"bytes"
	ascii2 "github.com/MinoIC/I2AW/image2ascii/ascii"
	"image"
	"image/color"
...
func NewImageConverter() *ImageConverter {
	return &ImageConverter{
		...
		pixelConverter: ascii2.NewPixelConverter(),
	}
}
...
type Converter interface {
	...
	Image2PixelASCIIMatrix(image image.Image, imageConvertOptions *Options) [][]ascii2.CharPixel
	ImageFile2PixelASCIIMatrix(image image.Image, imageConvertOptions *Options) [][]ascii2.CharPixel
}
...
type ImageConverter struct {
	...
	pixelConverter ascii2.PixelConverter
}

// Image2CharPixelMatrix convert a image to a pixel ascii matrix
func (converter *ImageConverter) Image2CharPixelMatrix(image image.Image, imageConvertOptions *Options) [][]ascii2.CharPixel {
	newImage := converter.resizeHandler.ScaleImage(image, imageConvertOptions)
	sz := newImage.Bounds()
	newWidth := sz.Max.X
	newHeight := sz.Max.Y
	pixelASCIIs := make([][]ascii2.CharPixel, 0, newHeight)
	for i := 0; i < int(newHeight); i++ {
		line := make([]ascii2.CharPixel, 0, newWidth)
		for j := 0; j < int(newWidth); j++ {
			pixel := color.NRGBAModel.Convert(newImage.At(j, i))
			// Convert the pixel to ascii char
			pixelConvertOptions := ascii2.NewOptions()
...
func (converter *ImageConverter) ImageFile2CharPixelMatrix(imageFilename string, imageConvertOptions *Options) [][]ascii2.CharPixel {
